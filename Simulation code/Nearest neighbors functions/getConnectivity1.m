function [fweights, fperimeter farea] = getConnectivity1(g)
...
for b=1:size(g.bonds,1),
    if(g.bonds(b,1) ~= 0),
        ...
        %weights(g.bonds(b,3),g.bonds(b,4)) = norm(pos(1,:)-pos(2,:))./(perimeter(g.bonds(b,3))*perimeter(g.bonds(b,4))); for the approximation
