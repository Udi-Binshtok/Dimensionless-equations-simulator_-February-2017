%function [ cell_location, cell_periodic_boundary_condition_location ] = Location( g, cell_number )
...
verts = g.bonds(bonds,1);                                   % verts = the inidicated number of one of the verts of each bond (all verts counted once that way)  
%verts_coordinates = g.verts(verts,1:2);                     % the coordinates of the verts as given by Micha
verts_coordinates = getRelativePosition(g,verts,cell_number); % the coordinates of the verts as given by Micha + correction
vector_average = sum(verts_coordinates,1)/length(verts);    % the position of the center of the cell
